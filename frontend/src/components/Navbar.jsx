  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-white shadow-sm items-center z-50'>
        ...
        {/* Mobile Sidebar */}
        <div className={`absolute md:hidden right-0 top-0 bottom-0 transition ${(isNavbarOpen ? 'translate-x-0' : 'translate-x-full')} bg-gray-100 shadow-md w-screen sm:w-9/12 h-screen`}>
          <div className='flex'>
          ...
          <ul className='flex flex-col gap-4 uppercase font-medium text-center'>
            ...
            {/* Show Admin Login only when not logged in */}
            {!authState.isLoggedIn && (
              <li className='py-2 px-3 text-primary hover:bg-gray-200 transition'>
